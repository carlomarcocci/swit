...
# il docker viene passati come parametro
# vengono backuppati tutti i database presenti sul server
######################################################################
...
MYSQL_SHOW_GRANTS_FOR_ALL="`dirname $0`/docker_mysql_show_grants_for_all.sh "

DIRW="${DIR}`date +%Y-%m-%d`"
FNAME=`date +%Y-%m-%d-%H-%M`
FILEOUT=${DIRW}/${HOST}_${FNAME}.out
FILEGRANTS=${DIRW}/${HOST}_${FNAME}_grants.sql
...
LIST_DB_TO_BACKUP=`${MYSQLCMD} --column-names=FALSE -B -e "SHOW DATABASES;" | grep -v -e "mysql" -e "performance_schema" -e "information_schema" -e "sys"`
